...
def load_jsonl_files(jsonl_dir):
    """
    加载指定目录下的所有jsonl文件并转换为QA格式
    
    Args:
        jsonl_dir: jsonl文件所在目录
    Returns:
        data: 所有数据的列表，每项都是QA格式的JSON字符串
    """
    data = []
    # 获取目录下所有的jsonl文件
    ...
    for file in jsonl_files:
        ...
        with open(file, "r", encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    # 转换为QA格式
                    qa_data = convert_to_qa_format(line)
                    if qa_data:  # 只添加成功转换的数据
                        data.append(json.dumps(qa_data, ensure_ascii=False) + "\n")
                except Exception as e:
                    print(f"警告：处理数据时出错: {str(e)}")
                    continue
        print(f"已读取并转换 {len(data)} 条数据")
        ...
        for item in train_data:
            ...
            try:
                ...
                if not isinstance(data, dict) or "question" not in data or "answer" not in data:
                    print(f"警告：跳过无效数据: {item[:100]}...")
                    continue
                f.write(item)
            except json.JSONDecodeError:
                print(f"警告：跳过无效JSON: {item[:100]}...")
                continue
